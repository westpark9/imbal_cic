...
class FallbackRouter(nn.Module):
    ...
    def __init__(self, num_experts=10, feature_dim=256):
...
class BaselineModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.backbone.fc = nn.Linear(in_features, 100)  # CIFAR-100 has 100 classes
...
class XGBoostModel:
    """XGBoost model wrapper for CIFAR-100 classification"""

    def __init__(self, use_features=True, n_components=512):
        """
        Args:
    ...
    def _extract_features(self, dataloader):
        ...
        with torch.no_grad():
            for data, targets in tqdm(dataloader, desc="Extracting XGBoost Features"):
...
class FinalExperiment:
    ...
    def __init__(self, num_experts=10, imbalance_factor=100, epochs=100):
        self.num_experts = num_experts
        self.imbalance_factor = imbalance_factor
        self.epochs = epochs
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        print("Final Clustering-Based Expert Ensemble Experiment - CIFAR-100")
    ...
    def setup_dataset(self, seed):
        """Setup CIFAR-100 with long-tail imbalance"""
    ...
    def _create_imbalanced_dataset(self, dataset):
        """Create long-tail imbalanced dataset for CIFAR-100"""
        targets = [dataset[i][1] for i in range(len(dataset))]

        # CIFAR-100 has fewer samples per class initially (500 vs 5000 in CIFAR-10)
        max_samples = 500
    ...
    def get_clustering_groups(self, seed):
        ...
        warmup_model.train()
        warmup_epochs = max(10, self.epochs // 7)  # Dynamic warmup based on total epochs
        for epoch in tqdm(range(warmup_epochs), desc="Warmup Training"):
        ...
        class_features = [[] for _ in range(100)]  # 100 classes
        train_loader = DataLoader(
            self.train_subset, batch_size=128, shuffle=False, num_workers=2
        )

        with torch.no_grad():
            for data, targets in tqdm(train_loader, desc="Collecting Class Features"):
                data = data.to(self.device)
        ...
        class_centroids = []
        for class_id in range(100):  # 100 classes
    ...
    def train_baseline(self, seed):
        ...
        for epoch in tqdm(range(self.epochs), desc="Training Baseline"):
            model.train()
            for data, targets in train_loader:
                data, targets = data.to(self.device), targets.to(self.device)
                optimizer.zero_grad()
    ...
    def train_xgboost(self, seed):
        ...
        # Create XGBoost model with higher n_components for CIFAR-100
        xgb_model = XGBoostModel(use_features=True, n_components=512)
        ...
        # Train XGBoost model - more trees for CIFAR-100's complexity
        xgb_model.model = xgb.XGBClassifier(
            n_estimators=500,  # More trees for 100 classes
    ...
    def train_expert_ensemble(self, expert_groups, seed):
        ...
        for epoch in tqdm(range(self.epochs), desc="Training Expert Ensemble"):
            ...
            for data, targets in train_loader:
                ...
                # Expert predictions
                final_logits = torch.zeros(data.size(0), 100, device=self.device)  # 100 classes
    ...
    def plot_confusion_matrix(self, cm, class_names, title, save_path, cmap="Blues"):
        """혼동 행렬을 시각화합니다."""
        # CIFAR-100은 클래스가 많아서 축소된 크기로 표시
    ...
    def plot_per_class_metrics(self, baseline_acc, baseline_f1, ensemble_acc, ensemble_f1, 
        ...
        plt.title('Per-Class F1 Score (CIFAR-100)')
        plt.ylabel('F1 Score')
        plt.xlabel('Class Index')
        plt.legend()
        plt.grid(True, alpha=0.3)
    ...
    def plot_model_comparison(self, baseline_acc, ensemble_acc, xgboost_acc, save_path):
        ...
        plt.title('Model Accuracy Comparison (CIFAR-100)', fontsize=14, fontweight='bold')
    ...
    def evaluate_model(self, model):
        ...
        model.eval()
        with torch.no_grad():
            for data, targets in tqdm(test_loader, desc="Evaluating Baseline"):
                data, targets = data.to(self.device), targets.to(self.device)
                outputs = model(data)
        ...
        per_class_acc, per_class_f1 = self.compute_per_class_metrics(
            all_predictions, all_targets, 100  # 100 classes
        )
    ...
    def evaluate_xgboost(self, xgb_model):
        ...
        # 클래스별 성능 지표
        per_class_acc, per_class_f1 = self.compute_per_class_metrics(
            predictions, test_targets, 100  # 100 classes
        )
    ...
    def evaluate_expert_ensemble(
        ...
        with torch.no_grad():
            for data, targets in tqdm(test_loader, desc="Evaluating Expert Ensemble"):
                data, targets = data.to(self.device), targets.to(self.device)
    ...
    def run_single_experiment(self, seed):
        ...
        # 실험 디렉토리 생성
        exp_dir = f"experiment_results_cifar100_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        # 결과 시각화 및 저장
        class_names = [f"Class_{i}" for i in range(100)]  # CIFAR-100 class names
        
        # 혼동 행렬 시각화
        self.plot_confusion_matrix(
            baseline_cm, class_names,
            "Baseline Model Confusion Matrix (CIFAR-100)",
            os.path.join(exp_dir, "baseline_confusion_matrix.png")
        )
        self.plot_confusion_matrix(
            xgboost_cm, class_names,
            "XGBoost Model Confusion Matrix (CIFAR-100)",
            os.path.join(exp_dir, "xgboost_confusion_matrix.png")
        )
        self.plot_confusion_matrix(
            ensemble_cm, class_names,
            "Ensemble Model Confusion Matrix (CIFAR-100)",
    ...
    def run_multiple_experiments(self, seeds):
        ...
        print(f"\n{'='*80}")
        print(f"FINAL RESULTS SUMMARY - CIFAR-100 ({len(seeds)} trials)")
        print(f"{'='*80}")
        print(
        ...
        results_file = (
            f"final_experiment_results_cifar100_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Final Clustering-Based Expert Ensemble Experiment - CIFAR-100"
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        "--num_experts", type=int, default=10, help="Number of expert networks (default: 10 for CIFAR-100)"
    )
    parser.add_argument(
        "--imbalance_factor",
        type=int,
    ...
    if args.mode == "fixed":
        ...
        # Save single result
        with open(f"final_experiment_cifar100_fixed_seed{args.seed}.json", "w") as f:
        ...
        with open(
            f"final_experiment_cifar100_combined_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
            "w",
        ) as f:
