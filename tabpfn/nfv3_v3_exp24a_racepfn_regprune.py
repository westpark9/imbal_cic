#!/usr/bin/env python3
"""EXP24A -- RACE-PFN, ONE KNOB vs exp23b: §12 marginal을 REGIME-
CONDITIONAL로 측정 (+ min-support), balanced anchor는 2,000으로 고정.

exp23b~23d의 교훈 (lab log 0826.md §5-§8): 균형 anchor(2k/class)는 회수층의
전제 조건이지만, tune 전체에서 재는 leave-one-out marginal을 0으로 눌러
고잔차 전문가(brute 질량 최대·inf·web)를 죽인다. anchor를 줄이면 전문가는
살지만(23d) 비전문 posterior가 무너져 시스템이 global 미달. 결론: anchor를
유지한 채 pruning의 "측정"을 고쳐야 한다.

THE KNOB: greedy_regime_prune() -- expert k의 marginal을 자기 regime에
hard-배정된 tune 행들 위에서만 잰다:
    marginal_k = mean_{i in regime_k} [ min_{bank−k} L_i  -  min_{bank} L_i ]
(balanced-loss 단위). 그 행들에선 "k 있음/없음" 양쪽에 anchor 기여가
동일하게 깔려 있어 상쇄되고 블록의 고유 가치만 남는다 -- 예: hard-brute
regime에서 brute 전문가 유무는 oracle ~0.92 vs anchor 상한 ~0.80 (exp23
실측 격차). greedy 반복(하나 제거 후 전원 재계산)은 유지. min-support:
regime의 masked tune 행이 --prune-min-regime-rows 미만이면 후보 제외
(23c의 9행 bot regime 방지). --prune-mode off는 coverage-생존 전원을
유지하는 ablation 스위치(대조군 run용). 문턱 --prune-min-regime-gain은
선언 상수(기본 0.01 loss). 그 외 exp23b와 동일. XGB는 skip(0.7548 인용).

Judged against (same run, same split): corrected global, dense oracle,
random/proximity banks, routing baselines; XGB 인용. Cross-run refs:
exp23b/23c/23d record runs (lab log).

    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    python tabpfn/nfv3_v3_exp24a_racepfn_regprune.py --target-dataset cic2018 \
        --dense-eval --skip-xgboost

smoke (capped suite): exp23 smoke args + --skip-xgboost
        (--prune-min-regime-rows 20 권장: capped 규모).
"""
...
def run_exp24a(args):
    ...
    args.experiment = "exp24a_racepfn_regprune"
    ...
    if args.train_split != "train":
        raise SystemExit("--train-split is not implemented in exp24a.")
    if args.skip_tabpfn:
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp24a_regprune")
