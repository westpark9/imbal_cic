#!/usr/bin/env python3
"""EXP20B -- exp20 with the TAIL expert SPLIT into separate inf / web experts.

Pre-registration: 0821.md SS23.  One structural knob vs exp20 (0.7235):

  candidates      = { global (natural 1M, plain classifier),
                      flood, bf_bot, inf, web }              (5, not 4)
  expert judgment = exp19-style conditional over {benign} U owned(k)
                    (context = owned rows + benign 200,000) -- single-class
                    experts (inf, web) fit fine: the benign block is the
                    second class, no filler needed
  selector input  = [ p_global(7) | p_flood(3) | p_bf_bot(3) | p_inf(2)
                      | p_web(2) ] = 17 dims, standardized
  selector        = 2-layer MLP(32), 5-way CE, target-balanced batches,
...
NO energy score anywhere.  Judged against exp20 (merged tail, 0.7235) and
exp21b (same split, energy selection).

    python tabpfn/nfv3_v3_exp20b_prob_split.py \
...
def run_exp20b(args):
    ...
    args.experiment = "exp20b_prob_split"
    ...
    all_rows = list(core.per_class_table("system_probsel_split", y_eval, final,
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp20b_probsplit")
