#!/usr/bin/env python3
"""EXP11c -- exp11b + ratio-invariant balanced gate loss (energy_ood-style).
...
Artifacts:
  ...
  scores_dump.npz                   per-class per-expert gate scores
    ...
    python tabpfn/nfv3_v3_exp11c_balanced_head.py --target-dataset cic2018 \\
...
def run_exp11(args):
    ...
    args.experiment = "exp11c_balanced_head"
    ...
    for ename, owned in experts.items():
        ...
        if ename == "benign":
            opp = np.concatenate([holdouts[e][0] for e in experts if e != "benign"])
        else:
            opp = holdouts["benign"][0]
        ...
        predict_head = train_score_head(f_hold, featurize(neg), args.seed,
