#!/usr/bin/env python3
"""EXP32 -- C0 BAGGING: 같은 구성의 C0 추첨 N개를 평균하는 global (exp31 + --c0-bags N).

    --c0-bags N   bag i (i=0..N-1)는 seed (args.seed + i) + SEED_BAND_GLOBAL 로 C0를 추첨하므로
                  bag i == exp30/31의 --seed (args.seed+i) 단독 run의 C0와 행 단위로 동일하다.
                  global 예측 = N개 TabPFN의 predict_proba 평균. 임베딩(phi)·anchor·PriorCorrector·
                  0g 감사표는 bag 0 기준. N=1이면 exp31과 코드 경로 동일.

왜: 0902.md §8l/§8p/§8x — 같은 구성에서 C0 추첨에 따라 hoic recall 0.74~0.93, macro ±0.01이 흔들린다(추첨 복권).
   100k S=0.75 균등 seed 42/43/44 = 0.7562/0.7617/0.7442. bags=3은 정확히 이 세 추첨의 평균 예측 → 앙상블이
   분산을 줄이고 평균을 올리는지, 그 비용(N배 추론, 100k에서는 1M 단일보다 여전히 싸다)이 정당한지 판독.

레인: python tabpfn/nfv3_v3_exp32_c0bag.py --target-dataset cic2018 --prune-mode off \
        --global-context-size 100000 --feasibility-banks none --skip-xgboost \
        --c0-benign-share 0.75 --c0-attack-alloc balanced --c0-bags 3
smoke: --target-dataset cic2018_capped --global-context-size 20000 + 위 인자 --c0-bags 2
base = exp31 사본. 추가: bag 추첨 루프, BaggedClf, timings.c0_bags / c0_bag_shares, context_rows C0_bag{i}.
"""
...
def run_exp29(args):
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp32_c0bag")
