#!/usr/bin/env python3
"""EXP34 -- C0 DRAW SELECTION: K개 C0 추첨 중 D_cal 점수가 가장 좋은 하나를 고른다 (exp32 + --c0-draw-select).

    --c0-bags K --c0-draw-select   K개 추첨(bag i = seed+i)을 각각 fit하고 D_cal(chrono상 test 이전, 클래스당
                                   ≤cal_cap 행)에서 benign recall과 macro-F1을 잰 뒤, benign recall ≥ 0.995인
                                   후보 중 macro 최대를 global로 채택(없으면 benign recall 최대). 평균(bagging)이
                                   아니라 선택이므로 추론 비용은 단일 run과 같다.

왜: 0902.md §8ad/§8af/§8ag — 같은 구성에서도 C0 추첨에 따라 benign→inf 붕괴(100k 균등 seed 46: 1.29M행)나 부분
    누수(250k 균등 seed 48: 44k행)가 1/7로 터진다. 붕괴는 D_cal(benign 2.1M·inf 22.6k)에서도 보여야 하므로 검증
    분할로 추첨을 고르는 것이 가장 싼 처방. SRC_HISTORY #2(검증 적합의 비전이) 위험은 선택 자유도가 K중 1이라 작다.

레인: python tabpfn/nfv3_v3_exp34_c0select.py --target-dataset cic2018 --prune-mode off \
        --global-context-size 100000 --feasibility-banks none --skip-xgboost \
        --c0-benign-share 0.75 --c0-attack-alloc balanced --c0-bags 5 --c0-draw-select --seed 46
smoke: --target-dataset cic2018_capped --global-context-size 20000 + 위 인자 --c0-bags 2 --c0-draw-select
base = exp32 사본. 추가: 선택 블록(global fit 지점), 0h_c0_draw_select.csv, timings.c0_draw_selected.
"""
...
def run_exp29(args):
    ...
    args.experiment = "exp34_c0select"
    ...
    bag_idx = [g_idx]                                    # THE KNOB (exp32)
    for bi in range(1, max(1, args.c0_bags)):
        sd = args.seed + bi + SEED_BAND_GLOBAL
        if share >= 0:
            gi = representative_subset_share(
                draw_pool, draw_y, n_classes, args.global_context_size, sd,
                benign_id0, share, alloc=args.c0_attack_alloc)
        else:
            gi = representative_subset(draw_pool, draw_y, n_classes,
                                       args.global_context_size, sd)
        bag_idx.append(gi)
    timings["c0_bags"] = int(max(1, args.c0_bags))
    if benign_id0 is not None:
        timings["c0_bag_shares"] = [
            round(float((label_fn(gi) == benign_id0).mean()), 6) for gi in bag_idx]
    # features for bags 1..N-1 must be materialised here: X is deleted later
    bag_ctx = [(feats_of(gi), label_fn(gi)) for gi in bag_idx[1:]]
    if len(bag_idx) > 1:
        print(f"C0 bags: {len(bag_idx)} draws of {args.global_context_size:,} "
              f"(seeds {[args.seed + i for i in range(len(bag_idx))]})", flush=True)
    ...
    t0 = time.time()
    class BaggedClf:
        """Mean predict_proba over N TabPFNs fitted on N C0 draws (exp32).
        classes_ spans all n_classes; embeddings come from bag 0."""
        ...
        def predict_proba(self, Xq):
            ...
            for m in self.members:
                pm = full_proba(np.asarray(m.predict_proba(Xq), dtype=np.float32),
                                m.classes_, n_classes)
                acc = pm if acc is None else acc + pm
            return acc / len(self.members)
        def get_embeddings(self, Xq, which="test"):
            return self.members[0].get_embeddings(Xq, which)
    draw_select_df = None
    if len(bag_idx) > 1 and args.c0_draw_select:          # THE KNOB (exp34)
        cands = [(glob_ctx[0], glob_ctx[1], g_idx)] + [
            (Xb, yb, gi) for (Xb, yb), gi in zip(bag_ctx, bag_idx[1:])]
        sel_rows, best, best_key = [], None, None
        for ci, (Xc, yc, gi) in enumerate(cands):
            clf_c = make_clf(Xc, yc)
            pc = batched_proba(clf_c, X_cal, f"draw{ci}/cal").argmax(axis=1)
            b_rec = (float((pc[y_cal == benign_id0] == benign_id0).mean())
                     if benign_id0 is not None else float("nan"))
            mf1 = float(f1_score(y_cal, pc, average="macro"))
            b2inf = int(((y_cal == benign_id0) & (pc != benign_id0)).sum()) \
                if benign_id0 is not None else -1
            sel_rows.append({"draw": ci, "seed": args.seed + ci,
                             "cal_benign_recall": round(b_rec, 6),
                             "cal_macro_f1": round(mf1, 6),
                             "cal_benign_misrouted": b2inf})
            key = (b_rec >= 0.995, mf1 if b_rec >= 0.995 else b_rec)
            if best is None or key > best_key:
                if best is not None:
                    del best[0]
                best, best_key = [clf_c, Xc, yc, gi, ci], key
            else:
                del clf_c
            free_gpu()
        glob, gX, gy, g_idx, sel = best
        glob_ctx = (gX, gy)
        draw_select_df = pd.DataFrame(sel_rows)
        draw_select_df["selected"] = draw_select_df["draw"] == sel
        timings["c0_draw_selected"] = int(sel)
        timings["c0_draw_selected_seed"] = int(args.seed + sel)
        print("C0 draw selection on D_cal:\n"
              + draw_select_df.to_string(index=False), flush=True)
    elif len(bag_idx) > 1:
        members = [make_clf(glob_ctx[0], glob_ctx[1])]
        for bi, (Xb, yb) in enumerate(bag_ctx, 1):
            members.append(make_clf(Xb, yb))
            print(f"  bag {bi} fitted: {len(yb):,} rows", flush=True)
        glob = BaggedClf(members)
    else:
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp34_c0select")
    ...
    for bi, gi in enumerate(bag_idx[1:], 1):          # exp32: extra C0 draws
        ctx_dump[f"C0_bag{bi}"] = gi
    if draw_select_df is not None:                     # exp34
        draw_select_df.to_csv(os.path.join(out_dir, "0h_c0_draw_select.csv"),
                              index=False)
...
def main():
    ...
    p.add_argument("--c0-attack-alloc", default="natural",
                   choices=["natural", "balanced"],
                   help="THE KNOB (exp31 A): split of the non-benign C0 "
                        "budget over attack classes.")
    p.add_argument("--c0-dedup", default="none", choices=["none", "exact"],
                   help="THE KNOB (exp31 B): drop exact (vector,label) "
                        "duplicates from the C0 draw pool before drawing.")
    p.add_argument("--c0-draw-select", action="store_true",
                   help="THE KNOB (exp34): among --c0-bags draws pick the one "
                        "with the best D_cal score instead of averaging.")
    p.add_argument("--c0-bags", type=int, default=1,
                   help="THE KNOB (exp32): number of C0 draws whose TabPFN "
                        "predict_proba are averaged for the global. bag i "
                        "uses seed (seed+i)+SEED_BAND_GLOBAL.")
    p.add_argument("--c0-conflict", default="keep", choices=["keep", "majority"],
                   help="THE KNOB (exp31 C): for feature vectors carrying "
                        ">1 label in the C0 pool keep only the majority-label "
                        "rows (ties -> lowest class id).")
