#!/usr/bin/env python3
"""EXP33 -- C0 DUP-CAP: (벡터,라벨) 사본 수 상한 (exp32 + --c0-dup-cap k).

    --c0-dup-cap k   C0 추첨 풀에서 같은 (feature 벡터, 라벨)의 사본을 최대 k개만 남긴다(첫 k개). 0 = 상한 없음(exp32).
                     dedup(k=1, §8m)은 hoic 0.99까지 열었지만 port-21 벡터 다수결과 benign 밀도를 파괴했다. k=5~20은
                     극단 사본 질량만 깎고 다수결·밀도 정보는 남기는지를 본다. dedup/conflict/bags 뒤에 적용되는 독립 knob.

레인: python tabpfn/nfv3_v3_exp33_c0dupcap.py --target-dataset cic2018 --prune-mode off \
        --global-context-size 100000 --feasibility-banks none --skip-xgboost \
        --c0-benign-share 0.75 --c0-attack-alloc balanced --c0-dup-cap 20
smoke: --target-dataset cic2018_capped --global-context-size 20000 + 위 인자 --c0-dup-cap 5
base = exp32 사본. 추가: dup-cap 블록(풀 단계), timings.c0_dupcap_rows_dropped.
"""
...
def run_exp29(args):
    ...
    args.experiment = "exp33_c0dupcap"
    ...
    if args.c0_dup_cap > 0:                              # THE KNOB (exp33)
        t0 = time.time()
        Xp = feats_of(draw_pool)
        hv = pd.util.hash_pandas_object(pd.DataFrame(Xp), index=False).values
        del Xp
        key = (hv.astype(np.uint64)
               ^ (draw_y.astype(np.uint64) * np.uint64(0x9E3779B97F4A7C15)))
        order = np.argsort(key, kind="stable")
        ks = key[order]
        first = np.r_[True, ks[1:] != ks[:-1]]
        grp_start = np.maximum.accumulate(np.where(first, np.arange(len(ks)), 0))
        rank = np.arange(len(ks)) - grp_start          # 0-based rank within group
        keep_sorted = rank < args.c0_dup_cap
        keep = np.zeros(len(key), dtype=bool); keep[order[keep_sorted]] = True
        timings["c0_dupcap_k"] = int(args.c0_dup_cap)
        timings["c0_dupcap_rows_dropped"] = int((~keep).sum())
        timings["c0_dupcap_s"] = round(time.time() - t0, 1)
        print(f"C0 pool dup-cap k={args.c0_dup_cap}: {len(draw_pool):,} -> "
              f"{int(keep.sum()):,} rows ({timings['c0_dupcap_s']}s)", flush=True)
        draw_pool, draw_y = draw_pool[keep], draw_y[keep]
        del hv, key, order, ks, first, grp_start, rank, keep_sorted, keep
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp33_c0dupcap")
...
def main():
    ...
    p.add_argument("--c0-dup-cap", type=int, default=0,
                   help="THE KNOB (exp33): keep at most k copies of each "
                        "(vector,label) in the C0 draw pool; 0 = no cap.")
