#!/usr/bin/env python3
"""EXP24B -- RACE-PFN, ONE KNOB vs exp24a: scorer의 학습 문제를
회귀(절대 이득) -> SIGN 분류 P(U>0)로 교체.

깔때기 진단 (report ⓪-2): exp23 계열의 확정 병목은 제안 관문 -- 이득-실재
행의 84~95%가 scorer·tau_pre에서 걸러진다(recall 0.05~0.16). 원인은
scorer가 6자릿수 스케일(w_bal 지배)의 U를 절대값 회귀하다 판별력을 잃는
것(실제 이득과 상관 ~0.03, top-1 일치 0.49~0.65).

THE KNOB: scorer = XGBClassifier(binary:logistic).
    label_ik  = 1[ U_ik > 0 ],  U = b_OOF·G − λ·c  (타깃 정의는 동일)
    weight_ik = log1p(|G_ik|)   (결과가 큰 행을 강조하되 스케일 압축)
    Û_ik      = P(U>0 | h_pre)  -- 행 간 비교 가능한 [0,1] 확률
tau_pre 선택(D_cal quantile grid)·verifier(quantile+보정)·그 외 전부
exp24a와 동일 (pruning = regime-conditional greedy, anchor 2,000 균형).
label이 단일 클래스로 퇴화하면 SystemExit(전 기록 규모에서 양성률 ~0.5).
--prune-mode off와 결합하면 "bank vs 관문" 원인 분리 대조군이 된다.
...
refs: exp24a record run (lab log 0827.md).

    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    python tabpfn/nfv3_v3_exp24b_racepfn_signscorer.py --target-dataset cic2018 \
        --dense-eval --skip-xgboost

smoke (capped suite): exp24a smoke args 동일.
"""
...
def run_exp24b(args):
    ...
    args.experiment = "exp24b_racepfn_signscorer"
    ...
    if args.train_split != "train":
        raise SystemExit("--train-split is not implemented in exp24b.")
    if args.skip_tabpfn:
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp24b_signscorer")
