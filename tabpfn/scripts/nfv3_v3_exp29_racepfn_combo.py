#!/usr/bin/env python3
"""EXP29 -- RACE-PFN COMBO ANCHOR: 개별 검증이 끝난 세 knob의 결합.

    exp27  --pool-partition scenario_stratified  (train+val 층화 분할)
    exp26  --verifier-target normgain            (verifier 타깃 = G / w_bal)
    exp28  --feas-match-memory per_expert        (등메모리 대조군 + regime_random)
    exp25b --det-precision {auto,float32}        (24GB 4090에서 결정성 유지)

왜 지금 combo인가. 세 knob은 각각 단독 run으로 검증이 끝났다:
    exp27: global 0.6698 -> 0.7277, brute 0.9159 (= XGBoost와 정확히 일치)
    exp26: ddos acceptance 0 -> 6,602, benign은 닫힌 채, harmful 56
    exp28: Gate 1을 메모리 위반 없는 비교로 재구성 (2f 감사표)
관례대로 개별 검증이 끝난 knob들의 결합 자체가 다음 실험이다. 특히 exp28의
Gate 1 재심은 결함 있는 분할(exp24b chrono, SSH가 C0에 0행) 위에서 내려졌으므로
수리된 분할 위에서 다시 판정해야 한다. 이 run이 이후 L3의 K=1 스윕과
lambda 스윕의 기준선(anchor)이 된다.

먼저 읽을 것:
    1. global vs xgb_c0 parity -- 층화 분할 위에서 corrected global이
       같은-run XGBoost와 클래스별로 어디까지 맞는가 (0e/ftp-vs-ssh 포함)
    2. inf/ddos acceptance -- normgain verifier가 층화 분할에서도
       ddos·infiltration을 여는가, benign FPR·harmful은 kill 기준 안인가
    3. equal-memory Gate 1 on the stratified split -- dense_oracle vs
       oracle_{random,proximity,regime_random}_bank (2f로 메모리 일치 감사)

base = exp26 사본 (normgain knob + exp25 계측: --deterministic, q_hat dump,
probs_xgboost.npz). 거기에 exp27의 분할 hunk, exp28의 등메모리 hunk,
exp25b의 --det-precision hunk를 이식했다. 네 knob 모두 기본값이 켜진 상태
(--prune-mode만 off를 명시해야 한다). exp26이 exp24b에서 fork되어
--dump-level은 없다 (stage dump가 필요하면 exp25 계열을 쓸 것).

record run (A100):
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    python tabpfn/nfv3_v3_exp29_racepfn_combo.py --target-dataset cic2018 \
        --prune-mode off --dense-eval --deterministic --det-precision float32
    (4090에서는 --det-precision auto -- float32는 OOM, exp25b run3)

smoke (capped suite):
    python tabpfn/nfv3_v3_exp29_racepfn_combo.py \
...
def run_exp29(args):
    ...
    args.experiment = "exp29_racepfn_combo"
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp29_combo")
