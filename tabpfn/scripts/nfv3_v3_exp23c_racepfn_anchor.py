#!/usr/bin/env python3
"""EXP23C -- RACE-PFN, ONE KNOB vs exp23b: shared anchor 2,000 -> 200
rows per class.

Root-cause chain (0826.md §5 보강): the shared anchor (2k/class, 12,761
rows in every expert context) makes every expert a passable balanced
generalist, which (a) collapsed §12 leave-one-out marginals (exp23: 7/8
pruned despite clearly distinct regimes), (b) drives the positive-
advantage ~90% crack tracked since exp22, and (c) rendered §7 prior
correction ineffective (beta=0 won). exp23b attacks (a) at the pruning
rule; THIS run attacks the root: a 10x smaller anchor keeps the §11.1
mandate (every class present in every context -- web ctx pool has 761
rows, so 200/class stays satisfiable) while shrinking the generalist
effect, so marginals and gains should re-differentiate and the block,
not the anchor, defines each expert.

THE KNOB: --anchor-per-class default 2000 -> 200. Everything else
byte-identical to exp23b (greedy backward pruning retained). XGBoost
skipped by run policy (cite 0.7548).
...
reference: exp23b record run (same night; see lab log 0826.md).

    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    python tabpfn/nfv3_v3_exp23c_racepfn_anchor.py --target-dataset cic2018 \
        --dense-eval --skip-xgboost

smoke (capped suite): exp23 smoke args with --anchor-per-class 30
        --skip-xgboost (10x-down mirror of the knob).
"""
...
def run_exp23c(args):
    ...
    args.experiment = "exp23c_racepfn_anchor"
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp23c_anchor")
