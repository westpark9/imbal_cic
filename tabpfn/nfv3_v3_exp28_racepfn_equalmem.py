#!/usr/bin/env python3
"""EXP28 -- RACE-PFN, ONE KNOB vs exp24b: §20.1 feasibility 대조군 bank를
residual bank와 **expert별 동일 메모리**로 맞춘다.

진단 (기준 run 20260827_174858_nfv3_cic2018_exp24b_signscorer,
2a_expert_contexts.csv). residual expert k는 min(regime 크기,
--expert-block-rows)행을 쓴다:
    15,468 / 20,718 / 6,322 / 130 / 186,000 = 228,638행
반면 random·proximity 대조군은 expert마다 무조건 args.expert_block_rows를 쓴다:
    5 x 186,000 = 930,000행  ->  930,000 / 228,638 = 4.07배 메모리 위반.
그러므로 현재의 Gate 1 판정(oracle_proximity_bank 0.9217 > dense_oracle
0.8294 > oracle_random_bank 0.7801)은 "residual context 선택이 열등하다"로도
"대조군이 4배 크다"로도 똑같이 설명된다 -- 해석 불가능한 비교다.
가이드 §20.1은 "Equal-memory random contexts"를 명시적으로 요구한다.

THE KNOB: --feas-match-memory per_expert
    대조군 bank의 블록 k가 정확히 len(experts[k]["block_rows"])행을 뽑는다.
    총량만 맞추는 것이 아니라 expert 하나하나를 개별로 맞춘다.
    off = exp24b 그대로 재현(모든 블록이 --expert-block-rows).

대조군 하나 추가: bank 'regime_random'
    regime k에 hard-assign된 행 **안에서만** 균등 무작위로, 같은 크기만큼 뽑는다.
    random(D_expert 전체)과 proximity(centroid 거리)는 "몇 행을, 어느 regime에서
    뽑았나"와 "diversity 선택기가 고른 행이 좋은가"를 분리하지 못한다.
    regime_random이 그 두 축을 가른다.

새 산출물: 2f_bank_block_sizes.csv (bank, expert, block_rows, regime_rows)
    -- 메모리 일치를 사후 감사할 수 있게 하는 근거표.

사전 등록 판정 기준 (가이드 §24 Gate 1 **재심**):
    equal-memory 상태에서 dense_oracle > oracle_proximity_bank AND
    dense_oracle > oracle_random_bank 이면 Gate 1 통과.
    이 run이 Gate 1을 다시 판정한다. 이전 Gate 1 수치(0.9217 / 0.8294 /
    0.7801)는 메모리가 어긋난 비교이므로 통과·불통과 어느 쪽으로도 인용 금지.
    regime_random은 별도 축이다: dense_oracle > oracle_regime_random_bank이면
    diversity 선택기가 값을 만든 것이고, 같으면 값은 regime 분할에서 나온 것이다.

Judged against (same run, same split): corrected global, dense oracle,
random/proximity/regime_random banks, routing baselines; XGB 인용(0.7548).
Cross-run refs: exp24b record run 20260827_174858 (lab log 0827.md) -- 단,
그 run의 bank 수치는 위의 이유로 비교 대상이 아니다.

    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    python tabpfn/nfv3_v3_exp28_racepfn_equalmem.py --target-dataset cic2018 \
        --dense-eval --skip-xgboost

smoke (capped suite):
    python tabpfn/nfv3_v3_exp28_racepfn_equalmem.py \
        --target-dataset cic2018_capped --dense-eval \
        --feasibility-banks random,proximity,regime_random
"""
...
def run_exp28(args):
    ...
    args.experiment = "exp28_racepfn_equalmem"
    ...
    if args.train_split != "train":
        raise SystemExit("--train-split is not implemented in exp28.")
    if args.skip_tabpfn:
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp28_equalmem")
