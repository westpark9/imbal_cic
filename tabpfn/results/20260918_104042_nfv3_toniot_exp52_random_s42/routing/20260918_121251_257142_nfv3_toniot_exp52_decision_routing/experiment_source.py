#!/usr/bin/env python3
"""EXP39: controlled routing ablations on an EXP38 frozen full-information cache.

The legacy control keeps EXP31's scorer labels and post features/quantile target.
All arms share a NEW macro-F1 threshold protocol, historical-prior standardization,
and chronological calibration/confirmation split. It is not an exact EXP31 rerun.
Success is measured on the original full, previously-used development test split.
No test labels enter fitting, class weights, thresholds or variant selection.

Arm order: legacy -> post_z or post_raw -> decision_post_raw -> decision_both_raw.
The last two change target/regression formulation, then scorer supervision.
Context/experts stay fixed. Dense cache creation is offline experimental work;
the deployable inference interface queries the scorer before calling one expert.
"""
import argparse
import gc
import hashlib
...
def run(args):
    ...
    out = Path(args.out_root) / (time.strftime('%Y%m%d_%H%M%S') +
                                f'_{os.getpid()}_nfv3_cic2018_exp39_decision_routing')
