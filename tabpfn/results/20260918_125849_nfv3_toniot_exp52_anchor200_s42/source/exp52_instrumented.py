#!/usr/bin/env python3
"""EXP52 -- fork of EXP31 (nfv3_v3_exp31_c0alloc.py, unmodified) adding ONE new
knob: --expert-select-mode. 09-18 진단(expert 전용 context가 자기 담당 클래스에서
오히려 global보다 나쁨, 최대 -0.543)의 원인 가설 검증용 -- diversity_select()가
군집 안에서도 residual 최상위(가장 헷갈린 사례)만 뽑던 것을, confident-first
(가장 확신하는 쉬운 사례) 또는 random으로 바꿔 self-class F1이 회복되는지 본다.
Global/anchor/K/block 예산 등 나머지는 전부 고정. 아래는 EXP31 원본 docstring.
...
def within_cell_pick(members, r_regime, k, mode, seed):
    """THE KNOB (exp52): which members of a diversity cell fill its budget.
    'residual' (default, byte-identical to exp31/exp39): hardest-first, i.e.
    the rows global is most wrong/uncertain on. 'confident': easiest-first,
    the rows global already gets right with high confidence -- the class's
    clean representative cases. 'random': uniform draw, no residual signal
    at all. Cluster-level mass_budget_alloc (how many rows per cell) is left
    untouched in all three modes -- only which rows fill a cell's quota
    changes, to isolate that one mechanism."""
    if k >= len(members):
        return members
    if mode == "residual":
        return members[np.argsort(-r_regime[members], kind="stable")[:k]]
    if mode == "confident":
        return members[np.argsort(r_regime[members], kind="stable")[:k]]
    if mode == "random":
        return np.random.default_rng(seed).choice(members, size=k, replace=False)
    raise ValueError(f"unknown expert_select_mode: {mode}")
...
def diversity_select(e_regime, r_regime, budget, n_sub, n_init, batch_size,
    ...
    if m <= 1:
        return np.sort(within_cell_pick(np.arange(n), r_regime, budget, mode, seed))
    ...
    for cid in range(m):
        ...
        if tgt[cid] <= 0 or len(members) == 0:
            continue
        top = within_cell_pick(members, r_regime, tgt[cid], mode, seed + 900_000 + cid)
        picked.append(top)
...
def run_exp29(args):
    ...
    args.experiment = "exp52_expert_context"
    ...
    _exp47_trace.record("inputs", locals())
    del X
    ...
    _exp47_trace.record("residual", locals())
    ...
    del e_tune_sig
    gc.collect()
    _exp47_trace.record("preprune", locals())
    ...
    _exp47_trace.record("route", locals())
    ...
    _exp47_trace.record("targets", locals())
    ...
    _exp47_trace.record("cal", locals())
    ...
    _exp47_trace.record("eval", locals())
    ...
    if args.dense_eval:
        ...
        for k in range(K):
            ...
            _exp47_trace.record("dense", locals())
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp52_expert_context")
    ...
    print(f"\nWrote {out_dir}")
    _exp47_trace.record("complete", locals())
    return out_dir
...
def main():
    ...
    p.add_argument("--expert-select-mode", default="residual",
                   choices=["residual", "confident", "random"],
                   help="THE KNOB (exp52): within-cell pick for expert block "
                        "rows -- residual=hardest-first (exp31 default, "
                        "byte-identical), confident=easiest-first (clean "
                        "class-representative cases), random=uniform draw. "
                        "Cluster-level mass_budget_alloc is unchanged in all "
                        "modes.")
