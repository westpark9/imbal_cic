...
def prepare_run(root):
    ...
    args.update(target_dataset='ton_iot', clean_manifest=str(Path(launch['clean_dir']) / 'manifest.json'),
                ...
                expert_select_mode=launch.get('expert_select_mode', 'residual'),
                anchor_per_class=launch.get('anchor_per_class', 2000))
    ...
    write_json(root / 'protocol.json', {'dataset': 'ton_iot', 'seed': 42,
        ...
        'training_configuration_changes': ['target_dataset: cic2018 -> ton_iot',
            f"expert_select_mode: residual (exp31 default) -> {launch.get('expert_select_mode', 'residual')}",
            f"anchor_per_class: 2000 (exp31 default) -> {launch.get('anchor_per_class', 2000)}"],
        'experiment_purpose': launch.get('experiment_purpose',
            '09-18 self-class specialization paradox diagnosis: does confident-first or '
