#!/usr/bin/env python3
"""EXP21B -- exp21 with the TAIL expert SPLIT into separate inf / web experts.

Pre-registration: 0821.md SS23.  One structural knob vs exp21 (0.7411):

  candidates = { global (natural 1M), flood, bf_bot, inf, web }   (5, not 4)
  selection  = plain 5-way argmax of energy-gate scores (no CE, no
               thresholds -- identical recipe to exp21)
  winner = global    -> global's 7-way argmax
  winner = attack k  -> conditional over owned(k) U {benign}
                        (context = owned + benign 200,000; return kept)

Disclosed second change (forced by the split, exp17 precedent): the GATE
contexts of the single-class experts (inf, web) must contain a second class
for TabPFN to fit, so each gets a benign filler block of --filler-rows
drawn at non-overlapping skip offsets of the benign off=0 permutation
(holdout occupies [0, holdout_rows)).  emb_ref (the similarity reference)
stays owned-only, and the conditional contexts are unchanged.

Judged against exp21 (merged tail, 0.7411) and exp20b (same split,
probability+CE selection).

    python tabpfn/nfv3_v3_exp21b_energy_split.py \
...
def run_exp21b(args):
    ...
    args.experiment = "exp21b_energy_split"
    ...
    experts = {}
    for ename, fams in SPLIT_GROUPS.items():
    ...
    # gate contexts/holdouts: exp12c reserve logic; single-class experts
    # get a benign filler block (exp17 precedent) at non-overlapping skips
    contexts, holdouts = {}, {}
    benign_fill_skip = args.holdout_rows          # [0, holdout_rows) = benign_hold
    for ename, owned in experts.items():
        ...
        if len(owned) == 1:                       # inf / web: fit needs 2 cls
            filler_idxs = np.sort(draw_class(benign_id, args.filler_rows,
                                             skip=benign_fill_skip))
            benign_fill_skip += args.filler_rows
    ...
    # ---- pass 1: energy gates for [global, flood, bf_bot, inf, web] ----
    ...
    all_rows = list(core.per_class_table("system_energy_split", y_eval, final,
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp21b_esplit")
