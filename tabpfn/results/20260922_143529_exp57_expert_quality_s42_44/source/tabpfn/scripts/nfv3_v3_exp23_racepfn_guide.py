#!/usr/bin/env python3
"""EXP23 -- RACE-PFN, guide-alignment package: everything the guide
(RACE-PFN_RESEARCH_IMPLEMENTATION_GUIDE.md) prescribes for the SYSTEM that
exp22c had not yet implemented. exp22c (run 20260826_130310, macro 0.7428 /
global 0.6800 / XGB 0.7548 / oracle 0.8480) proved Phase-1 complementarity
(oracle > random bank +0.075); this script adds the remaining phases.

DECLARED MULTI-PART PACKAGE (multi-knob by design; each layer has an
ablation flag or grid so later one-knob runs can decompose it):

  P0  §5 six-way split: D_global/D_expert/D_route from train (50/25/25,
      chrono per class, exp22c-compatible) + D_tune/D_cal carved from the
      previously RESERVED val block (chrono per class, first 40% tune /
      last 60% cal => pre-test ratios ~37/19/19/10/15 vs guide 40/20/15/
      10/15). Test stays the 4.02M development holdout (§18 terminology).
      0c split manifest (rows + time range per split*class). §18 rename:
      D_mine -> D_expert (code + CLI).
  P2  §7 prior correction, two-stage on D_tune: stage 1 selects T on the
      GLOBAL balanced NLL (C0 prior == pi_ref by construction, so beta is
      unidentifiable there -- smoke 20260826_171819 measured this); stage 2
      selects beta per candidate bank on the EXPERT balanced NLL (anchor-
      balanced expert priors are what §7 targets; beta=0 row = mandated
      no-correction ablation). 1c = T grid, 1d = per-K expert beta grid.
      p~0 uses (0, T) everywhere; expert posteriors use (beta, T).
  P2  §10 K selection: --k-candidates (default 2,4,8) banks built and
      scored by oracle macro-F1 on D_tune; best kept (2d).
  P2  §12 pruning: expert dropped if tune positive-gain coverage <
      --prune-min-coverage OR marginal oracle macro on D_tune <=
      --prune-min-marginal (2e). At least one expert survives.
  P3  §15 shared utility scorer replaces the hard K+1 router: one XGB
      regressor over (query, expert) pairs, h_pre = [p0,H,m,z,d_k,a0,q_k]
      with expert descriptor q_k = [log1p(block), regime mass, mean r,
      prior entropy, log1p(mean d2), cost]; target = OOF realized policy
      utility U = b_OOF*G - lambda*c, where b_OOF comes from 2-fold
      chronological OOF quantile verifiers on D_route (wasted calls are
      learned, guide §15).
  P4  §14 verifier: shared LOWER-QUANTILE regressor (xgboost
      reg:quantileerror, --verifier-quantile) on pure predictive gain G
      (NO cost term -- the §18 separation; cost lives only in the scorer),
      h_post with q_k instead of a k-onehot. One-sided correction on
      D_cal: G_lower = q_hat - Q_{1-delta}(q_hat - G). tau_pre and
      tau_post fixed ON D_CAL (§5 table) by grid search maximizing net
      realized utility subject to the §14 constraint set: benign-FPR
      increase (true replacement delta) <= --cal-benign-fpr-increase,
      harmful fraction <= --cal-harmful-frac, proposal rate <=
      --cal-max-proposal, minimum support --cal-min-accepted /
      --cal-min-decided (4c grid, 4d coverage).
  P0+ §5 duplicate ENFORCEMENT: NetFlow rows repeat verbatim across splits,
      so beta/T, K selection and pruning exclude D_tune rows content-
      duplicated in C0/anchor/D_expert, and the calibration/threshold
      statistics exclude D_cal rows content-duplicated in D_route (masked
      counts logged; per-class-empty fallback recorded). Pairwise dup-hash
      matrix in 0d; source_file/session IDs absent from the pkl schema is
      recorded as a data limitation.
  §16 inference: p0 -> corrected -> scorer top-1 (call iff U_hat >
      tau_pre) -> expert -> corrected -> G_lower > tau_post -> override.
  §21 routing baselines from the dense pass at zero extra cost (rows in
      per_class_metrics + 6f summary): nearest-centroid-always and
      scorer-top1-no-verifier, next to the system, dense oracle and the
      Gate-1 banks.

Phase-1 machinery (residual clip / full signature / hard assignment /
diversity coreset) is byte-identical to exp22c, except residuals are now
computed from the prior-CORRECTED global posterior (guide §8 uses p~0).
All exp22c §20+/§22/§23/§25 records are kept (transitions, FP accounting,
guardrails, activation, Jaccard, agreement, gain corr/coverage, context
priors, row-ID/membership/gain dumps + sha256, dup-hash overlap, env,
peak-GPU, feasibility banks).

Still deferred (experiment-matrix scale, not system code): §17/§20.5
temporal refresh, §23 multi-seed + block-bootstrap CI, §20.3 IF sweep,
§20.4 prior-shift suites, §20.6 batch-latency percentiles, §21 static
baseline zoo (CatBoost/DistPFN/LoCalPFN/MixturePFN/Drift-Resilient).

Judged against (same run, same split): corrected global TabPFN, raw
global, full-pool XGBoost, dense oracle, random/proximity banks, routing
baselines. Cross-run reference: exp22c run 20260826_130310.

    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    python tabpfn/nfv3_v3_exp23_racepfn_guide.py --target-dataset cic2018 \
        --dense-eval

smoke (capped suite):
    ... --target-dataset cic2018_capped --global-context-size 20000 \
        --anchor-per-class 300 --expert-block-rows 4000 \
        --expert-max-rows 60000 --route-cap-per-class 3000 \
        --tune-cap-per-class 2000 --cal-cap-per-class 2000 \
        --phi-fit-rows 60000 --kmeans-max-rows 60000 \
        --diversity-subclusters 256 --k-candidates 2,4 \
...
def run_exp23(args):
    ...
    args.experiment = "exp23_racepfn_guide"
    ...
    if args.train_split != "train":
        raise SystemExit("--train-split is not implemented in exp23.")
    if args.skip_tabpfn:
    ...
    # ---- P2 §12: pruning on D_tune -------------------------------------
    G_t = L_t[:, 0:1] - L_t[:, 1:]
    msk_t = tune_sel_mask
    full_pick = L_t.argmin(axis=1)
    y_or_full = np.take_along_axis(np.stack(best["preds_t"], axis=1),
                                   full_pick[:, None], axis=1)[:, 0]
    mac_full = float(f1_score(y_tune[msk_t], y_or_full[msk_t],
                              average="macro"))
    prune_rows, keep = [], []
    for k in range(K):
        cov = float((G_t[msk_t, k] > 0).mean())
        cols = [0] + [j + 1 for j in range(K) if j != k]
        sub_lab = np.stack([best["preds_t"][j] for j in cols], axis=1)
        y_or_sub = np.take_along_axis(
            sub_lab, L_t[:, cols].argmin(axis=1)[:, None], axis=1)[:, 0]
        marginal = mac_full - float(f1_score(y_tune[msk_t], y_or_sub[msk_t],
                                             average="macro"))
        kept = (cov >= args.prune_min_coverage
                and marginal > args.prune_min_marginal)
        prune_rows.append({"expert": k + 1, "tune_pos_coverage": round(cov, 4),
                           "tune_marginal_oracle_macro": round(marginal, 4),
                           "kept": kept})
        if kept:
            keep.append(k)
    if not keep:   # keep the single best marginal
        keep = [int(np.argmax([r["tune_marginal_oracle_macro"]
                               for r in prune_rows]))]
        prune_rows[keep[0]]["kept"] = True
    ...
    scorer.fit(np.concatenate(hpre_rt),
               np.concatenate([U_rt[:, k] for k in range(K)])
               .astype(np.float32))
    ...
    U_hat_tu = np.stack(
        [scorer.predict(build_pair_pre(p0_tune, z_tune, np.sqrt(d2_tu[:, k]),
                                       a0_tu, qk_mat[k])) for k in range(K)],
        axis=1)
    ver_rows = []
    for k in range(K):
    ...
    U_hat_cal = np.stack(
        [scorer.predict(build_pair_pre(p0_cal, z_cal, np.sqrt(d2_cal[:, k]),
                                       a0_cal, qk_mat[k])) for k in range(K)],
        axis=1)
    ...
    U_hat_ev = np.stack(
        [scorer.predict(build_pair_pre(p0_eval, z_eval,
                                       np.sqrt(d2_eval[:, k]), a0_eval,
                                       qk_mat[k])) for k in range(K)], axis=1)
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp23_raceguide")
...
def main():
    ...
    p.add_argument("--prune-min-marginal", type=float, default=0.001,
                   help="§12: min tune marginal oracle macro to keep")
