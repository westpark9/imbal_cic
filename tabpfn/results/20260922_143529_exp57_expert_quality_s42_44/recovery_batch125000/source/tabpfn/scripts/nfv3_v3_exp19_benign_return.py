#!/usr/bin/env python3
"""EXP19 -- benign RETURN exit in the conditional stage (user structure).

Pre-registration: 0821.md SS16.  One knob vs exp12c: each ATTACK expert's
conditional classifier gains a benign block in its context, so a row that was
mis-assigned to that expert can be classified back to benign ("return")
instead of being force-converted into an owned class.  The GATE pass is
UNCHANGED (same contexts, same heads, same argmax) -- only the pass-2
conditional contexts and label space change:

  conditional context(k) = owned(k) rows (exp12c와 동일 draw)
                           + benign 200,000 (별도 draw, off=50)
  label(k-won row)       = argmax over owned(k) U {benign}
  benign-expert win      = label benign (변경 없음)

Nothing is learned from predictions (no #2 exposure); no thresholds.
The global reference is refit in-run (natural 1M, seed band 990).
...
distinct, ~x1.9 compute saving; also removes SS12n batch-boundary jitter).
    ...
    python tabpfn/nfv3_v3_exp19_benign_return.py --target-dataset cic2018 \\
...
def run_exp19(args):
    ...
    args.experiment = "exp19_benign_return"
    ...
    # THE KNOB: benign block for the attack conditionals (off=50 -> disjoint
    # from the benign expert's own context draw)
    ...
    # ---- pass 1: gate scores (UNCHANGED vs exp12c; distinct-broadcast) ----
    enames = list(experts)
    ...
    # ---- pass 2: conditionals WITH the benign return exit ----
    ...
    all_rows = list(core.per_class_table("system_return", y_eval, final,
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp19_return")
