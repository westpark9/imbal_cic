...
def prepare(root,workspace,datasets=None,seeds=None,data=None,checkpoint=None,clean_root=None,test_batch_size=None,cpu_threads=16,portable_inputs=False):
    ...
    jobs=[]
    datasets=datasets or list(bases);seeds=seeds or [42,43,44]
    for seed in seeds:
        for ds,base in bases.items():
            ...
            if data is not None:args.update(data=str(Path(data).resolve()),data_dir=str(Path(data).resolve().parent))
            if checkpoint is not None:args['model_path']=str(Path(checkpoint).resolve())
            if clean_root is not None:args['clean_manifest']=str(Path(clean_root).resolve()/Path(args['clean_manifest']).parent.name/'manifest.json')
            if test_batch_size is not None:args['test_batch_size']=test_batch_size
            job=dict(id=f'{ds}_s{seed}',dataset=ds,seed=seed,K=4,bootstrap_repeats=200,args=args,cpu_threads=cpu_threads,portable_inputs=portable_inputs)
            write(job_dir/'job.json',job);jobs.append(job['id'])
    write(root/'protocol.json',dict(experiment='EXP57',jobs=jobs,K=4,seeds=seeds,arms=['designed','matched_random','balanced_random'],
...
def worker(root,job_id):
    ...
    cpu_threads=int(job.get('cpu_threads',16))
    import torch
    torch.set_num_threads(cpu_threads)
    torch.set_num_interop_threads(min(cpu_threads,4))
    install_clean_loader(core,job['args']['clean_manifest'],source_override=job['args']['data'] if job.get('portable_inputs') else None)
    ...
    try:
        with threadpool_limits(cpu_threads):module.run_exp29(types.SimpleNamespace(**job['args']))
