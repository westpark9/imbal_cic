#!/usr/bin/env python3
"""EXP47: clean CIC2018, run original EXP31, then existing EXP39 legacy policy."""
import argparse
import ast
...
from cic2018_conflict_clean import install_clean_loader, sha256, write_json
from exp47_trace import Trace, instrument
...
def baseline(root, launch):
    ...
    module = types.ModuleType('exp47_instrumented_baseline')
...
def prepare_run(root):
    ...
    args.update(clean_manifest=str(Path(launch['clean_dir']) / 'manifest.json'),
    ...
    sources = [Path(__file__).resolve(), HERE / 'cic2018_conflict_clean.py', HERE / 'exp47_trace.py', HERE / 'exp47_audit.py',
    ...
    write_json(root / 'protocol.json', {'dataset': 'cic2018', 'seed': 42,
        'parent_args': str(source_run / 'args.json'), 'clean_manifest': args['clean_manifest'],
        'training_configuration_changes': [], 'diagnostic_changes': ['dense_eval=True', 'nine observation callbacks', 'calibration all-expert predictions after baseline test'],
