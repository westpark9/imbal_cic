#!/usr/bin/env python3
"""EXP11b -- exp11 + mixed negatives (aux + opposite-side holdouts).  (user regrouping, closed-set)
...
Artifacts:
  ...
  scores_dump.npz                   per-class per-expert gate scores
    ...
    python tabpfn/nfv3_v3_exp11b_activation_negatives.py --target-dataset cic2018 \\
...
def run_exp11(args):
    ...
    args.experiment = "exp11b_activation_negatives"
    ...
    for ename, owned in experts.items():
        ...
        if ename == "benign":
            opp = np.concatenate([holdouts[e] for e in experts if e != "benign"])
        else:
            opp = holdouts["benign"]
        rng_n = np.random.default_rng(args.seed + 4242)
        opp = opp[rng_n.permutation(len(opp))[: args.aux_train_rows]]
        neg = np.concatenate([aux_train, opp])
        predict_head = train_score_head(featurize(holdouts[ename]),
                                        featurize(neg), args.seed)
        id_holdout_scores[ename] = predict_head(featurize(holdouts[ename]))
        gate[ename] = {}
