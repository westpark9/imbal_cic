...
1 global; 2 observable fixed-region assignment; 3 each expert on the full test;
4 saved scorer with matched calls, no verifier; 5 saved scorer+verifier.
Observable region assignment never consumes truth. Fixed-FPR thresholds are
written before loading test y; the historical class map is descriptive only.
...
def assignment_oracle(y, global_pred, experts, mapping):
    """Historical diagnostic only: true-class selection is unsuitable for quality claims."""
    owner = mapping[y]
    called = owner >= 0
...
def fixed_region_assignment(experts, distances):
    """Select a fixed observable centroid; no truth, correctness, or fallback inputs."""
    if experts.ndim != 2 or experts.shape != distances.shape or experts.shape[1] == 0:
        raise ValueError('Predictions/distances must have the same nonempty N x K shape')
    if not np.isfinite(distances).all():
        raise ValueError('Centroid distances must be finite')
    owner = distances.argmin(axis=1)
    return experts[np.arange(len(experts)), owner], owner
...
def main():
    ...
    dump(out / 'protocol.json', {
        ...
        'historical_mapping_rule': 'diagnostic only: block dominant class; largest target-class block; lowest ID breaks ties',
        'condition_2': 'argmin observable centroid distance; raw expert output; no test truth or fallback',
        'historical_assignment_oracle': 'excluded from main comparison: true class reveals target information',
        'expert_count_constraint': 'K < C for newly generated banks',
        'bank_satisfies_expert_count_constraint': K < C,
        'condition_2_unassigned_rows': 'none; every row uses its closest observable centroid',
    ...
    fixed, owners = fixed_region_assignment(E, np.load(cache / 'eval_distance.npy', mmap_mode='r'))
    all_rows = np.ones(N, bool)
    no_rows = np.zeros(N, bool)
    policies = [('global', 1, g, no_rows, no_rows),
                ('fixed_region', 2, fixed, all_rows, all_rows)]
    ...
    for label, condition, pred, calls, accept in policies:
        ...
        if label in ('fixed_region', 'scorer_only', 'scorer_verifier'):
