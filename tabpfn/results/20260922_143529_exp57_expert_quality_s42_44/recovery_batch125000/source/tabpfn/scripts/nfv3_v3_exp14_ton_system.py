#!/usr/bin/env python3
"""EXP14 -- the exp12c activation-expert system transferred to ton_iot.

Pre-registration: 0821.md SS4.  One knob vs exp12c: the TARGET DATASET
(cic2018 -> ton_iot).  The method is unchanged: per-expert gates (frozen
similarity features -> learned score head, 1:2 pos:neg, class-uniform
positives), argmax activation, winner-only conditional classification.
Three consequences of the dataset switch, disclosed up front:

  grouping   read from configs/nfv3_experts.json (the s43/s44 record) plus a
             benign expert: flood={ddos,dos}, probe_access={scanning,password},
             application={injection,xss}, stealth_malware={backdoor,mitm,
             ransomware}.  5 experts instead of 4.
  aux        mirrored recipe "attack families of the OTHER suite datasets":
             cic2018 target used [bot_iot, ton_iot]; ton_iot target uses
             [bot_iot, cic2018].  Name-overlapping families (ddos, dos) drop
             out automatically; (bot_iot, theft) stays excluded.
  global     ton has no recorded global dump, so the reference column is
             fitted IN THIS RUN (0820.md SS12n: comparison columns must be
             same-run recomputations): natural-proportion context of
             --global-context-size train rows, same n_estimators as the
             experts, probs saved for reuse.

    python tabpfn/nfv3_v3_exp14_ton_system.py --target-dataset ton_iot \\
...
AUX_MAP = {
    "cic2018": ["bot_iot", "ton_iot"],
    "ton_iot": ["bot_iot", "cic2018"],
    "bot_iot": ["ton_iot", "cic2018"],
}
AUX_EXCLUDE = {("ton_iot", "scanning"), ("bot_iot", "theft")}
# json keys of configs/nfv3_experts.json per cfg target (без _capped)
EXPERT_JSON_KEY = {"cic2018": "cse_cic_ids2018", "ton_iot": "ton_iot",
                   "bot_iot": "bot_iot"}
...
def load_aux_rows(args, cfg, target_families, cap_per_family, seed):
    ...
    base = args.target_dataset.replace("_capped", "")
    for ds in [d + suffix for d in AUX_MAP[base]]:
        a = copy.copy(args)
        a.target_dataset = ds
...
def run_exp14(args):
    ...
    args.experiment = "exp14_ton_system"
    ...
    base = args.target_dataset.replace("_capped", "")
    with open(args.expert_config, encoding="utf-8") as f:
        groups_json = json.load(f)[EXPERT_JSON_KEY[base]]
    groups = {"benign": ["benign"], **groups_json}
    ...
    for ename, (idxs, _) in contexts.items():
        lab = label_fn(idxs)
        comp = {class_names[c]: int(n) for c, n in
                zip(*np.unique(lab, return_counts=True))}
        print(f"  composition[{ename}]: {comp}")

    # global reference context drawn BEFORE X is freed
    glob_ctx = None
    if args.fit_global:
    ...
    print("aux composition:\n" + aux_meta.to_string(index=False))
    core._PICKLE_CACHE.clear()
    gc.collect()

    # recorded-dump alternative to --fit-global
    p_glob = None
    ...
    # ---- global reference fitted in the same run ----
    out_dir_probs = None
    if glob_ctx is not None and p_glob is None:
        t0 = time.time()
        Xg, yg = glob_ctx
        clf = TabPFNClassifier(
            device=args.device, model_path=args.model_path,
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp14_system")
...
def main():
    ...
    p.add_argument("--fit-global", action="store_true",
                   help="fit the natural-proportion global reference in this "
                        "run (ton has no recorded dump)")
    p.add_argument("--global-context-size", type=int, default=1_000_000)
    p.add_argument("--expert-config",
                   default=os.path.join(core.REPO_ROOT, "configs",
                                        "nfv3_experts.json"))
    p.set_defaults(
        target_dataset="ton_iot",
        max_train_samples=-1,
        n_estimators=1,
