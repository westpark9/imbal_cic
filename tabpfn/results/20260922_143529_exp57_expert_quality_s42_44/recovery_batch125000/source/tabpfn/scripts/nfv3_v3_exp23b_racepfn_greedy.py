#!/usr/bin/env python3
"""EXP23B -- RACE-PFN, ONE KNOB vs exp23: §12 pruning single-pass ->
GREEDY BACKWARD elimination.

exp23 (run 20260826_183417, macro 0.7001 / global 0.6677 / oracle 0.8237)
exposed the flaw: the K=8 bank's regimes were clearly distinct (7a: brute-
pure 17.6k with the LARGEST residual mass 0.336, inf-pure 15.6k, web-pure
140, dos-pure 21k, ...), yet single-pass leave-one-out marginals collapsed
to ~0 for 7/8 experts because the shared anchor (2k/class) makes every
expert a passable generalist -- each one is individually replaceable while
the bank is collectively essential (removing the sole survivor e1 still
left tune oracle at 0.9496). The pruned brute/inf/web specialists map 1:1
to the classes where the system loses to XGBoost. Lab log: 0826.md §5.

THE KNOB: greedy_backward_prune() -- remove ONE smallest-marginal expert
at a time and RECOMPUTE every marginal from the stored tune loss matrix
after each removal; stop when all survivors clear --prune-min-marginal.
A specialist survives at the moment it becomes the last expert covering
its failure mode (its marginal then jumps to the anchor-vs-block gap,
e.g. brute ~0.92 vs ~0.80). Coverage viability applied once up front;
per-round marginals recorded in 2e. Threshold values unchanged
(--prune-min-marginal 0.001, --prune-min-coverage 0.005 -- declared
constants). Everything else byte-identical to exp23. XGBoost baseline is
skipped by run policy (--skip-xgboost; cite 0.7548 from exp23/exp22c).

Judged against (same run, same split): corrected global, raw global,
dense oracle, random/proximity banks, routing baselines; XGB cited.
Cross-run reference: exp23 run 20260826_183417.

    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    python tabpfn/nfv3_v3_exp23b_racepfn_greedy.py --target-dataset cic2018 \
        --dense-eval --skip-xgboost

smoke (capped suite): exp23's smoke args + --skip-xgboost.
"""
...
def run_exp23b(args):
    ...
    args.experiment = "exp23b_racepfn_greedy"
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp23b_greedy")
