#!/usr/bin/env python3
"""EXP51: scorer-target ablation on an EXP39-format frozen cache (S1V0, S1V1),
forked from EXP50 (nfv3_v3_exp50_hdn_verifier.py, unmodified) to add the S1
scorer arm from docs/research/20260916/expert_acceptance_next_plan.md §1.

Arms (kind, scorer_kind, target_kind):
- legacy (S0V0): existing OOF sign-classification scorer (indirect target --
  does the cross-fitted NLL-gain teacher's predicted sign agree with the real
  outcome), weighted-NLL-gain quantile-regression verifier. Control.
- hdn (S0V1): same S0 scorer, verifier replaced by a 3-class classifier over
  {harmful, neutral, helpful} = sign(decision_gain), scored as
  P(helpful) - hdn_lambda * P(harmful).
- s1v0 (S1V0): scorer replaced by EXP39's pre-existing but previously-unused
  'decision' scorer_kind -- a direct mean-regression of decision_gain (+1
  helpful / -1 harmful / 0 neutral) from the same pre-call features, i.e.
  E[decision_gain] = P(helpful) - P(harmful) targeted directly instead of via
  the NLL-teacher-agreement proxy. Paired with the unmodified V0 (nll) verifier
  target -- isolates the scorer-target effect alone. No new training code: this
  scorer was already implemented (SPECS 'decision_post_raw'/'decision_both_raw'
  in EXP39) but never paired with 'base' kind + 'nll' verifier target.
- s1v1 (S1V1): S1 scorer + V1 (hdn) verifier -- both changes together.

Verifier models are keyed only by (kind, target_kind) and are shared/reused
across scorer arms within one run (e.g. legacy and s1v0 both use
verifier_base_nll.joblib) -- no redundant training.
All other mechanics (threshold search, chronological cal split, evaluation) are
byte-identical to EXP39/EXP50.
"""
import argparse
import gc
import hashlib
...
SPECS = {
    ...
    's1v0': ('base', 'decision', 'nll'),  # S1V0: direct decision-gain scorer, V0 (nll) verifier
    's1v1': ('base', 'decision', 'hdn'),  # S1V1: direct decision-gain scorer, V1 (hdn) verifier
}
...
def run(args):
    ...
    out = Path(args.out_root) / (time.strftime('%Y%m%d_%H%M%S') +
                                f'_{os.getpid()}_exp51_scorer_target')
