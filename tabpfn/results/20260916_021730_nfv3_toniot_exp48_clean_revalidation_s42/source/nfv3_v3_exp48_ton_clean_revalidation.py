#!/usr/bin/env python3
"""EXP48: clean ToN-IoT, run original EXP31, then existing EXP39 legacy policy."""
import argparse
import ast
...
def baseline(root, launch):
    ...
    module = types.ModuleType('exp48_instrumented_baseline')
...
def routing(root):
    ...
    source = source.replace('_nfv3_cic2018_exp39_decision_routing', '_nfv3_toniot_exp39_decision_routing')
    (root / 'source/exp39_output_name_only.py').write_text(source)
    exp = types.ModuleType('exp48_legacy_routing')
...
def prepare_run(root):
    ...
    sources = [Path(__file__).resolve(), HERE / 'nfv3_conflict_clean.py', HERE / 'cic2018_conflict_clean.py', HERE / 'exp48_trace.py', HERE / 'exp47_trace.py', HERE / 'exp47_audit.py',
    ...
    write_json(root / 'protocol.json', {'dataset': 'ton_iot', 'seed': 42,
        ...
        'training_configuration_changes': ['target_dataset: cic2018 -> ton_iot', 'dataset tail vocabulary: mitm,ransomware'], 'secondary_protected_classes': ['ddos', 'dos'], 'diagnostic_changes': ['dense_eval=True', 'nine observation callbacks', 'calibration all-expert predictions after baseline test'],
