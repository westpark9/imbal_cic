#!/usr/bin/env python3
"""EXP25 -- RACE-PFN, **ZERO KNOB**: exp24b(--prune-mode off)와 결과가 같아야 하는
계측 전용 fork. 목적은 (a) E-2 잡음 대역 측정, (b) 지금 저장되지 않아 사후 복원이
불가능한 값들의 계측(0831.md §2.1).

기본 실행은 exp24b --prune-mode off와 **동작이 동일**하다. 아래 셋은 전부 opt-in 플래그이고
기본값에서는 켜지지 않는다.

계측 (항상 켜짐, 결과에 영향 없음)
  * probs_xgboost.npz      -- XGB의 test posterior. 지금은 저장되지 않아 selective/
                              cost/temporal 축에서 XGB와 비교가 아예 불가능하다.
  * system_dump.q_hat      -- 보정 **전** verifier 출력. 지금은 g_lower(=q̂ − q_corr)만
                              있어 "예측이 나쁜 것"과 "보정이 센 것"을 사후 분리할 수 없다.
  * timings.batch_latency  -- 단계별 배치 지연 p50/p95/p99와 rows/s. 0831 §1에서 확인된
                              실측 compute 배수 2.04(보고값 1.16이 아님)를 run 안에서
                              직접 재기 위한 것.
  * --dump-level full      -- D_expert/route/tune/cal의 global posterior p0를 저장.
                              GPU 없이 재분석할 수 있고, 나중에 --reuse-global-probs
                              (E-2의 캐시 경로)를 붙일 수 있게 한다.

opt-in 플래그 (기본 off = exp24b와 동일)
  * --deterministic        -- E-2의 핵심 시험. 같은 C0·같은 ckpt·같은 eval 행인데도
                              global macro가 8개 run에서 0.6461~0.6734로 흔들리고
                              (prior_T=1.25, beta=0으로 전 run 동일하므로 prior 탓이
                              아니다), 행 집합이 sha256까지 같아도 residual이 99.93%
                              행에서 달라 작은 regime 구성이 바뀐다. 이 플래그는
                              inference_precision=float32 + cudnn/tf32 결정성 +
                              CUBLAS_WORKSPACE_CONFIG로 그 대역이 접히는지 본다.
                              접히면 캐시 없이 재현성이 확보된다.
  * --reuse-context-rows P -- 다른 run의 context_rows.npz에서 C0/anchor/phi_fit/
                              expert/route/tune/cal/eval 행 ID를 그대로 가져온다.
                              (행 고정만으로는 부족하다는 것이 이미 확인됐지만,
                              --deterministic과 함께 쓰면 두 원인을 분리할 수 있다.)

E-2 실행 (같은 박스에서 연속으로):
    python tabpfn/nfv3_v3_exp25_racepfn_dump.py --target-dataset cic2018 \
        --prune-mode off --dense-eval --dump-level full          # 반복 1
    (동일 커맨드 반복 2)
    python tabpfn/nfv3_v3_exp25_racepfn_dump.py --target-dataset cic2018 \
        --prune-mode off --dense-eval --dump-level full --deterministic

판정: paired delta(system − corrected global)의 표준편차. --deterministic에서
global macro 대역이 < 0.005로 접히면 이후 전 실험의 기본값으로 채택한다.

smoke (capped suite):
    python tabpfn/nfv3_v3_exp25_racepfn_dump.py --target-dataset cic2018_capped \
...
def run_exp25(args):
    ...
    args.experiment = "exp25_racepfn_dump"
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp25_dump")
