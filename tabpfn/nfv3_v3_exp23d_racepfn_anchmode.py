#!/usr/bin/env python3
"""EXP23D -- RACE-PFN, ONE KNOB vs exp23c: the anchor's CLASS COMPOSITION
(--anchor-mode), budget-matched at per_class * C rows.

exp23c shrank the shared anchor (2000 -> 200/class) but kept it EQUAL per
class. This script varies what the anchor is made of, holding the total
budget fixed, to separate "anchor size" from "anchor balance" in the
generalist effect that collapses §12 marginals and drives the positive-
advantage crack:

  equal    min(per_class, n_c) per class          [= exp23c, reproduce]
  natural  total budget at the D_global pool's natural class ratios
           (>= 1 row per present class) -- expert context prior ~= global
           prior, so any balanced-context advantage disappears and gains
           must come from the specialist block alone
  inverse  quota ∝ 1/n_c (rare classes get the most rows), capped at the
           pool count, >= 1 per class -- the opposite extreme: maximal
           tail support in every context

Everything else byte-identical to exp23c (greedy backward pruning, anchor
budget parameter --anchor-per-class 200). One mode per run; the Args dump
is the record. XGBoost skipped by run policy (cite 0.7548).

Judged against (same run, same split): corrected global, dense oracle,
random/proximity banks, routing baselines; XGB cited. Cross-run
references: exp23b/exp23c record runs (same night, lab log 0826.md).

    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    python tabpfn/nfv3_v3_exp23d_racepfn_anchmode.py --target-dataset cic2018 \
        --anchor-mode natural --dense-eval --skip-xgboost

smoke (capped suite): exp23 smoke args with --anchor-per-class 30
        --anchor-mode natural --skip-xgboost.
"""
...
def anchor_subset_mode(pool_idx, pool_y, n_classes, per_class, mode, seed):
    """THE KNOB: anchor composition, budget-matched at per_class * C_present.
    equal reproduces exp23c; natural follows the pool's class ratios
    (>=1/class); inverse gives quota ∝ 1/n_c (rare classes most), capped."""
    if mode == "equal":
        return anchor_subset(pool_idx, pool_y, n_classes, per_class, seed)
    counts = np.bincount(pool_y, minlength=n_classes)
    present = counts > 0
    total = per_class * int(present.sum())
    if mode == "natural":
        return representative_subset(pool_idx, pool_y, n_classes, total, seed)
    w = np.zeros(n_classes, dtype=np.float64)
    w[present] = 1.0 / counts[present]
    raw = w / w.sum() * total
    tgt = np.minimum(np.floor(raw).astype(np.int64), counts)
    tgt[present & (tgt < 1)] = 1
    tgt = np.minimum(tgt, counts)
    rem = total - int(tgt.sum())
    while rem > 0:
        progressed = False
...
def greedy_backward_prune(L_t, preds_arr, y, mask, min_coverage,
                          min_marginal, f1_fn):
    """§12 as greedy backward elimination (THE KNOB): coverage viability
    once up front, then repeatedly drop the smallest-marginal expert and
    RECOMPUTE all marginals, until every survivor clears min_marginal.
    L_t columns = [global, e1..eK]; preds_arr aligned. Returns
    (sorted keep list, per-round record DataFrame)."""
    K = L_t.shape[1] - 1
    G = L_t[:, 0:1] - L_t[:, 1:]
    cov = {k: float((G[mask, k] > 0).mean()) for k in range(K)}

    def oracle_macro(cols):
        pick = L_t[:, cols].argmin(axis=1)
        y_or = np.take_along_axis(preds_arr[:, cols], pick[:, None],
                                  axis=1)[:, 0]
        return f1_fn(y[mask], y_or[mask])
    ...
    for k in range(K):
        if cov[k] >= min_coverage:
            ...
            rows.append({"round": 0, "expert": k + 1,
                         "tune_pos_coverage": round(cov[k], 4),
                         "marginal": np.nan, "action": "dropped(coverage)"})
    rnd = 1
    while len(active) > 1:
        full = oracle_macro([0] + [k + 1 for k in active])
        margs = {k: full - oracle_macro([0] + [j + 1 for j in active
                                               if j != k]) for k in active}
        worst = min(active, key=lambda k: (margs[k], cov[k]))
        removing = margs[worst] <= min_marginal
    ...
    if not active:
        ...
        active = [best_k]
        rows.append({"round": rnd, "expert": best_k + 1,
                     "tune_pos_coverage": round(cov[best_k], 4),
                     "marginal": np.nan, "action": "kept(fallback)"})
...
def run_exp23d(args):
    ...
    args.experiment = "exp23d_racepfn_anchmode"
    ...
    anchor_idx = anchor_subset_mode(ctx_pool, y_ctx_pool, n_classes,
                                    args.anchor_per_class, args.anchor_mode,
    ...
    # ---- P2 §12 (THE KNOB): greedy backward pruning on D_tune ----------
    G_t = L_t[:, 0:1] - L_t[:, 1:]
    preds_arr = np.stack(best["preds_t"], axis=1)
    keep, prune_df = greedy_backward_prune(
        L_t, preds_arr, y_tune, tune_sel_mask,
        args.prune_min_coverage, args.prune_min_marginal,
        lambda a, b: float(f1_score(a, b, average="macro")))
    del preds_arr
    print("greedy backward pruning (§12):\n"
    ...
    out_dir = os.path.join(
        args.out_root,
        f"{ts}_{cfg[args.target_dataset]['out_tag']}_exp23d_anchmode"
        f"_{args.anchor_mode}")
...
def main():
    ...
    p.add_argument("--anchor-mode", default="equal",
                   choices=["equal", "natural", "inverse"],
                   help="THE KNOB: anchor class composition at fixed total "
                        "budget per_class*C (equal = exp23c)")
    p.add_argument("--anchor-per-class", type=int, default=200,
                   help="THE KNOB: 2,000 -> 200 (10x down); anchor still covers every class (§11.1)")
