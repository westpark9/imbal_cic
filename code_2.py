#!/usr/bin/env python3
"""
code_2.py — Additive Boost Binary Specialist MoE
...
[MoE 구조 다이어그램]
  ...
  score[tail T]     = P_base[T] + α_T × P_spec(T|x)
  prediction        = argmax_c score[c]

  α_T ∈ [0, 3.0]: val set에서 class T의 F1을 최대화하는 값으로 per-class tuning.
    α_T=0   → baseline만 사용 (specialist 무시)
    α_T>1   → specialist가 baseline 스케일을 능가하는 부스트 가능

  [code_1 대비 핵심 변경]
  code_1 공식(β=1.0): score[T] = P_spec ≈ 0.55
    → base_probs[benign]=0.70이 그대로여서 argmax가 여전히 benign
    → brute-force recall 0.85→0.22 붕괴
  code_2 공식: score[T] = P_base[T] + α × P_spec
    → α=1.5이면 0.10 + 1.5×0.55 = 0.925 > 0.70 → T 승리
    → α>1 허용으로 표현력 확장 (이전 β∈[0,1] 제약 해소)
...
[Tail class 선정 기준 (Union)]
  ...
  단, train count < MIN_SPECIALIST_TRAIN (default 30)이면 specialist 훈련 스킵.
  (sql-injection 13개, infiltration 22개 → 이진 분류기 학습 불가 → 스킵)

[학습 순서]
  1. make_splits: train / val / test 분할
  2. BaselineXGB.fit(X_train, y_train)
  3. select_tail_classes(y_train, X_val, y_val, baseline)
  4. BinarySpecialist.fit(X_train, y_train)  ← train_n >= 30인 tail class만
  5. tune_alphas(X_val, y_val)              ← per-class α 최적화 (범위 [0, 3.0])
  6. predict(X_test)                        ← additive boost 후 argmax
═══════════════════════════════════════════════════════════════════════════════
"""
...
class ConfidenceGatedMoE:
    ...
    [Gate 설계 원리]
      ...
      α_T(large): 강한 부스트 → recall 향상, false positive 위험 증가
      α_T=0:      baseline만 사용

    [α_T 결정 방식]
      val에서 class T에 대한 F1을 최대화하는 α_T를 그리드서치.
      탐색 범위 [0, 3.0]: α>1 허용으로 specialist가 dominant해지는 구간까지 탐색.
      val positive 샘플이 MIN_VAL_POS개 미만이면 DEFAULT_ALPHA 사용.
    """

    ALPHA_GRID = np.round(np.arange(0.0, 3.1, 0.1), 1)  # [0.0, 0.1, ..., 3.0]
    MIN_VAL_POS = 3           # α tuning 최소 val positive 수
    DEFAULT_ALPHA = 1.0       # val positive 부족 시 기본 α
    MIN_SPECIALIST_TRAIN = 30  # train 샘플 이하이면 specialist 훈련 스킵
    ...
    def fit_specialists(
        ...
        for c in self.tail_classes:
            n_pos = int(counts[c])
            if n_pos < self.MIN_SPECIALIST_TRAIN:
                logger.info(
                    f"  [SKIP:{self.class_names[c]}] train_n={n_pos} < "
                    f"{self.MIN_SPECIALIST_TRAIN} → specialist 훈련 불가"
                )
                continue
    ...
    def tune_alphas(
        ...
        """
        Val에서 각 tail class T의 F1을 최대화하는 α_T를 결정.

        score[T] = P_base[T](x) + α × P_spec(T|x)   (additive boost)
        → α를 [0, 3.0] 범위에서 grid search하여 val F1@T 최대화.
        α>1 허용: specialist 신호가 baseline 스케일을 넘어설 수 있음.
        """
        logger.info("=== [4/4] α per-class tuning ===")
        assert self.baseline is not None and self.baseline.is_trained
        ...
        for c, specialist in self.specialists.items():
            ...
            if n_pos < self.MIN_VAL_POS:
                self.alphas[c] = self.DEFAULT_ALPHA
                logger.info(
                    f"  {self.class_names[c]:30s} | val_pos={n_pos} < {self.MIN_VAL_POS} "
                    f"→ α=DEFAULT({self.DEFAULT_ALPHA})"
                )
                continue
            ...
            for alpha in self.ALPHA_GRID:
                ...
                scores[:, c] = base_probs_val[:, c] + alpha * p_spec_val
            ...
            self.alphas[c] = best_alpha
            logger.info(
                f"  {self.class_names[c]:30s} | val_pos={n_pos:4d} "
                f"→ α={best_alpha:.2f} (val F1={best_f1:.4f})"
            )
    ...
    def predict(
        ...
        [추론 흐름]
          ...
          3. score 결합: score[T] = P_base[T] + α_T * P_spec(T|x)  (additive boost)
          4. argmax → final prediction

        Returns
        -------
        preds : (N,) int
        info  : dict with 'base_probs', 'specialist_conf', 'alphas'
        """
        assert self.baseline is not None and self.baseline.is_trained
        ...
        for c, specialist in self.specialists.items():
            if not specialist.is_trained:
                continue
            alpha = self.alphas.get(c, 0.0)
            if alpha == 0.0:
                continue
            p_spec = specialist.predict_confidence(X, batch_size=batch_size)
            spec_conf[c] = p_spec
            final_scores[:, c] = base_probs[:, c] + alpha * p_spec
...
def per_class_comparison(
    ...
    return pd.DataFrame({
        ...
        "alpha":       [alphas.get(c, float("nan")) for c in labels],
...
def main():
    ...
    exp_dir = f"results/code2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    if args.model in (1, 2):
        ...
        # Step 4: α tuning
        moe.tune_alphas(X_val, y_val, logger)
    ...
    if args.model in (1, 2) and moe.specialists:
        ...
        cmp_df = per_class_comparison(
            ...
            num_classes, class_names, moe.tail_classes, moe.alphas,
        )
