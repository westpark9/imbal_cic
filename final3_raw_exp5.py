...
class FinalExperiment:
    ...
    def get_clustering_groups(self, seed):
        ...
        if use_frequency_only:
            # Frequency-only 모드: 원본 샘플 수에 로그 변환 적용하여 더 균등한 분포 생성
            log_samples = np.log1p(samples_array)  # log(1+x)로 0값 방지
            
            # 로그 변환된 값을 정규화 (0-1 범위)
            min_log = log_samples.min()
            max_log = log_samples.max()
            normalized_log = (log_samples - min_log) / (max_log - min_log)
            
            # 0-1 범위를 0-4 범위로 확장 (더 큰 차이 생성)
            scaled_features = normalized_log * 4.0
            
            frequency_features = np.repeat(scaled_features.reshape(-1, 1), 20, axis=1)
            combined_features = frequency_features
            
            print(f"       Using frequency-only clustering with log scaling (20 dimensions)")
            print(f"       Original samples range: [{samples_array.min():.0f}, {samples_array.max():.0f}]")
            print(f"       Log samples range: [{log_samples.min():.3f}, {log_samples.max():.3f}]")
            print(f"       Scaled features range: [{scaled_features.min():.3f}, {scaled_features.max():.3f}]")
        else:
            # Option 2: Frequency 극도로 강화 + class centroids 최소화
            frequency_weight = 1000.0   # 극강 가중치
            frequency_features = np.repeat(frequency_zscore.reshape(-1, 1), 100, axis=1) * frequency_weight
            
            # Class centroids는 최소한으로만 사용 (2차원으로 극도로 압축)
            pca = PCA(n_components=2)
            reduced_centroids = pca.fit_transform(class_centroids)
            scaler = StandardScaler()
            reduced_centroids_scaled = scaler.fit_transform(reduced_centroids)
            
            combined_features = np.concatenate([frequency_features, reduced_centroids_scaled], axis=1)
            print(f"       Using frequency-dominant clustering (100 freq dims + 2 feature dims)")
