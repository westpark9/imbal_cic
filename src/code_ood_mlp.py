#!/usr/bin/env python3
"""
Chronological MLP variant for CIC-IDS2017 OOD/TTA experiments.
...
import code_ood_v1 as base
BASE_PREDICT_PROBA = base._predict_proba
BASE_PREDICT_LOGITS = base._predict_logits
...
class TorchMLP(nn.Module):
    def __init__(self, input_dim, n_classes, hidden_dims, dropout):
        ...
        for hidden in hidden_dims:
            layers.extend([nn.Linear(prev, hidden), nn.LayerNorm(hidden),
...
class MLPClassifierWrapper:
    def __init__(self, input_dim, n_classes, hidden_dims, dropout, lr,
        ...
        self.model = TorchMLP(input_dim, n_classes, self.hidden_dims, dropout).to(self.device)

    def fit(self, X, y, sample_weight=None, eval_set=None, verbose=False):
        torch.manual_seed(self.seed)
        Xs = self.scaler.fit_transform(np.asarray(X, dtype=np.float32)).astype(np.float32)
        ...
        if eval_set:
            ...
            if len(Xv_np) > 0:
                Xv = torch.from_numpy(
                    self.scaler.transform(np.asarray(Xv_np, dtype=np.float32)).astype(np.float32)
                ).to(self.device)
                yv = torch.from_numpy(np.asarray(yv_np, dtype=np.int64)).to(self.device)
        ...
        for epoch in range(self.epochs):
            ...
            val_loss = self._eval_loss(Xv, yv) if Xv is not None else float(loss.detach().cpu())
    ...
    def _eval_loss(self, Xv, yv):
        ...
        with torch.no_grad():
            ...
            for start in range(0, len(Xv), self.batch_size * 4):
                ...
                loss = F.cross_entropy(logits, yv[start:start + self.batch_size * 4],
                                       reduction="sum")
    ...
    def predict_logits(self, X, chunk=50_000):
        ...
        with torch.no_grad():
            for start in range(0, len(Xs), chunk):
                ...
                outs.append(self.model(xb).detach().cpu().numpy().astype(np.float32))
...
def _predict_proba(model, X, chunk=50_000):
    ...
    return BASE_PREDICT_PROBA(model, X, chunk)


def _predict_logits(model, X, n_cls, chunk=50_000):
    if len(X) == 0:
        return np.zeros((0, n_cls), dtype=np.float32)
    if isinstance(model, MLPClassifierWrapper):
        return model.predict_logits(X, chunk)
    return BASE_PREDICT_LOGITS(model, X, n_cls, chunk)


base._predict_proba = _predict_proba
base._predict_logits = _predict_logits
expert_raw_scores = base.expert_raw_scores
compute_expert_signals = base.compute_expert_signals
...
def train_mlp_model(X_tr, y_tr, X_va, y_va, n_cls, args, device, seed, log, name):
    ...
    model.fit(X_tr, y_tr, sample_weight=base._balanced_w(y_tr),
...
def train_local_classifier(X_tr, y_tr, X_va, y_va, global_classes,
    ...
    model = train_mlp_model(X_tr[tr_mask], yl_tr, X_va[va_mask], yl_va,
                            len(global_classes), CURRENT_ARGS, device, seed, log, name)
...
def train_experts(X_tr, y_tr, X_va, y_va, partitions, device,
    ...
    if gate not in ("energy", "imood_energy"):
        raise ValueError("code_ood_mlp currently supports --ood_gate energy or imood_energy")
    experts = []
    for e, (name, classes) in enumerate(partitions):
        log.info(f"Training MLP expert {e}: {name} ({len(classes)} classes)")
        local, prior = train_local_classifier(
            X_tr, y_tr, X_va, y_va, classes, device, seed + e, log, name)
        expert = Expert(name, classes, local, None, 0.0, 1.0, prior)
        calib_X, calib_y = X_va, y_va
        if not np.isin(y_va, classes).any():
            log.warning(f"  no owned validation rows for {name}; calibrating threshold on train rows")
            calib_X, calib_y = X_tr, y_tr
        if threshold_mode == "hard_negative":
            tau, scale = base.calibrate_threshold_hard_negative(
                expert, calib_X, calib_y, gate, q, alpha, benign_id,
        ...
        elif threshold_mode == "per_class_min":
            tau, scale = base.calibrate_threshold_per_class(expert, calib_X, calib_y, gate, q, alpha)
        else:
            tau, scale = base.calibrate_threshold(expert, calib_X, calib_y, gate, q, alpha)
...
def save_mlp_metadata(bundle, args, out_dir):
    ...
    pd.DataFrame(rows).to_csv(os.path.join(out_dir, "mlp_metadata.csv"), index=False)
...
def main():
    ...
    out_dir = make_unique_out_dir("results", f"{ts}_pid{os.getpid()}_chrono_mlp")
    log = setup_logger(os.path.join(out_dir, "experiment.log"))
    log.info(f"Args: {vars(args)}")
    logging.getLogger("ood_tta_gate").info("Running chronological original-label MLP")
    ...
    log.info(
        f"Loaded chrono MLP data: train={len(y_tr):,} val={len(y_va):,} "
    ...
    save_baseline_ood_absorption_by_class(
        y_test_ood_original, y_base_ood, original_class_names, out_dir)
    save_mlp_metadata(bundle, args, out_dir)
