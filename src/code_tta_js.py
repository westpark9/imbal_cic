#!/usr/bin/env python3
"""
code_tta_js.py — Exclusive-partition MoE + JS-divergence stability TTA.

Expert design:
  Each expert EXCLUSIVELY owns a disjoint class subset.
  - Single-class expert (Benign): binary detector trained on ALL data (benign=1 vs rest=0).
  - Multi-class experts: XGBoost trained ONLY on their own class samples.
  Within-expert inverse-frequency weights handle local imbalance.
  Local probability vectors are zero-padded to global class space for aggregation.

TTA — per-sample JS stability:
  ...
  JS measures *consistency* across augmented views.
  An expert whose prediction flips between views gets low weight.

Baseline: XGBoost with global balanced sample weights.

Usage:
  python src/code_tta_js.py --data cic2017_proc.pkl --model 2
"""
import argparse
import gc
import logging
...
# ── exclusive partition patterns ────────────────────────────────────────────
# Order matters: first match wins. "Tail" is filled by IR threshold.
PARTITIONS_2017 = [
    ("Benign",     ["benign"]),
    ("DoS",        ["dos-"]),
...
class ExpertConfig:
    name: str
    global_class_ids: list   # sorted global class IDs owned by this expert
    is_binary: bool          # True → single-class → binary benign detector
    # Mapping: local idx i → global_class_ids[i]
    # Binary exception: local idx 1 (positive) → global_class_ids[0]


# ── logging ─────────────────────────────────────────────────────────────────

def setup_logger(path):
    log = logging.getLogger("tta_js")
...
def train_expert(X_tr, y_tr, X_va, y_va, cfg, n_est, device, seed, log):
    t0 = time.time()
    if cfg.is_binary:
        # All-data binary: positive = owned class (benign), negative = all attacks
        ...
        # Exclusive multiclass: train ONLY on own-class samples
...
def zero_pad(local_p, cfg, n_global):
    """Embed local probability vector into global class space."""
    N = len(local_p)
    g = np.zeros((N, n_global), dtype=np.float32)
    if cfg.is_binary:
        # local index 1 = positive (owned) class
...
# ── JS divergence stability ───────────────────────────────────────────────────

def _js_batch(p, q, eps=1e-10):
    """Per-sample Jensen-Shannon divergence. [N,C],[N,C] → [N]."""
    p = np.clip(p, eps, 1)
    q = np.clip(q, eps, 1)
    m = 0.5 * (p + q)
...
def compute_js_weights(models, configs, X_b, n_views, p_mask, rng):
    """
    Per-sample per-expert weights via JS stability.

    stability_e(x) = 1 / (1 + (1/K) Σ_k JS(p_e(x) ‖ p_e(x'_k)))
    weights         = row-normalised stability matrix  [B, E]

    Returns:
        weights   [B, E]  — sum to 1 per row
        base_local         list of [B, C_e] local probs (original x)
    """
    B, E = len(X_b), len(models)
    base = [_predict_proba(m, X_b, chunk=B) for m in models]
    js   = np.zeros((B, E), dtype=np.float64)

    for _ in range(n_views):
        Xa = vime_perturb(X_b, p_mask, rng)
        for e, m in enumerate(models):
            js[:, e] += _js_batch(base[e], _predict_proba(m, Xa, chunk=B))

    stab = 1.0 / (1.0 + js / n_views)          # [B, E]  higher = more stable
...
def tta_predict(models, configs, X_te, n_global, n_views, p_mask, seed,
    ...
    """
    Per-sample adaptive aggregation via JS stability.
    ...
    for s in range(0, len(X_te), batch_size):
        ...
        w, base = compute_js_weights(models, configs, Xb, n_views, p_mask, rng)
        ws.append(w)
...
def save_weight_heatmap(weight_matrix, configs, y_te, class_names, out_dir, log):
    """
    Heatmap: rows = true class (sorted by support desc),
             cols = expert,
             cell = mean JS-stability weight that expert received for
                    samples of that true class.

    Diagnostic: if the JS signal works, each expert should get high weight
    for its own class subset and low weight for others.
    """
    E   = len(configs)
    n_c = len(class_names)
    ...
    # mean weight per true-class per expert
    ...
    ax.set_title("Mean JS-stability weight per true class × expert", fontsize=11)
...
def main():
    parser = argparse.ArgumentParser(
        description="Exclusive-partition MoE + JS-stability TTA for IDS")
    parser.add_argument("--data",         required=True)
    parser.add_argument("--seed",         type=int,   default=42)
    parser.add_argument("--n_estimators", type=int,   default=300)
    parser.add_argument("--n_views",      type=int,   default=5,
                        help="VIME augmented views for JS stability (default 5)")
    ...
    if args.model in (1, 2):
        ...
        t1 = time.time()
        log.info(f"JS-stability TTA: n_views={args.n_views}, p_mask={args.p_mask}")
    ...
    if args.model == 2 and y_pred_b is not None and y_pred_m is not None:
        ...
        save_colored_table(rows, col_headers, png,
                           title=f"Exclusive MoE + JS-TTA | seed={args.seed}")
        log.info(f"PNG → {png}")
