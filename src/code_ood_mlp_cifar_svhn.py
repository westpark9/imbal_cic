...
class FlattenMLP(nn.Module):
    def __init__(self, num_classes, hidden_dims, dropout):
        super().__init__()
        layers = []
        prev = 3 * 32 * 32
...
def build_mlp(num_classes, args):
    return FlattenMLP(
...
def train_mlp(model, train_loader, val_loader, args, device, log):
    model.to(device)
...
def save_classification(y_true, y_pred, class_names, out_dir):
    ...
    render_table_png(
        ...
        title="CIFAR-10 MLP classification")
...
def save_ood_summary(metrics, threshold, scale, args, out_dir):
    ...
    render_table_png(
        ...
        title="CIFAR-10 MLP vs SVHN energy OOD summary")
...
def main():
    ...
    out_dir = make_unique_out_dir(
        args.out_root, f"{ts}_pid{os.getpid()}_cifar10_svhn_mlp_energy")
    ...
    t0 = time.time()
    model = build_mlp(len(class_names), args)
    model = train_mlp(model, train_loader, val_loader, args, device, log)
