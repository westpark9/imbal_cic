...
def assign_window_ratio_split(n: int, window_size: int) -> np.ndarray:
    if n == 0:
        return np.array([], dtype=object)

    idx = np.arange(n, dtype=np.int64)
    window_ids = idx // max(int(window_size), 1)
    slot = window_ids % 10
    return np.where(slot < 6, "train", np.where(slot < 8, "val", "test")).astype(object)
...
def load_and_preprocess_from_csv_dir(
    ...
    monday_train_ratio = float(np.clip(monday_train_ratio, 0.0, 1.0))
    logger.info(
        f"CSV chronological split config: window_size={window_size}, monday_train_ratio={monday_train_ratio:.3f}"
    )
    ...
    for file_idx, file_name in enumerate(csv_files):
        ...
        local_split = np.empty(n_rows, dtype=object)
        # CIC2017 only: first CSV (Monday) can be train-only with optional downsampling.
        if dataset_type == "cic2017" and file_idx == 0:
            keep_n = int(np.floor(n_rows * monday_train_ratio))
            if monday_train_ratio > 0.0 and keep_n == 0:
                keep_n = 1
            keep_n = min(keep_n, n_rows)
            if keep_n == 0:
                logger.warning("First CIC2017 CSV configured to keep 0 rows (monday_train_ratio=0). Skipping.")
                continue
            if keep_n < n_rows:
                df = df.iloc[:keep_n].reset_index(drop=True)
                n_rows = keep_n
            local_split[:] = "train"
            logger.info(f"CIC2017 first CSV train-only applied: kept_rows={n_rows}")
        else:
            # Per-file chronological window split: 6:2:2 by window index.
            local_split = assign_window_ratio_split(n_rows, window_size=window_size)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to raw CSV directory. For CIC2017/2018 this uses chronological window-based split.",
    )
    parser.add_argument(
        "--window_size",
        type=int,
        default=200,
        help="Window size used for chronological split assignment on non-fixed CSV rows.",
    )
    parser.add_argument(
        "--monday_train_ratio",
        type=float,
        default=1.0,
        help="CIC2017 only: keep this ratio of the 1st CSV rows as train-only.",
    )
    ...
    if args.data_dir:
        data = load_and_preprocess_from_csv_dir(
            ...
            monday_train_ratio=args.monday_train_ratio,
        )
