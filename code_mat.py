...
def infer_dataset_type(data_path, data=None):
    ...
    if "2018" in name:
        return "cic2018"
    raise ValueError(
        f"Cannot infer dataset_type from '{data_path}'. Include unswnb15/cic2017/cic2018 in filename or in pickle['dataset_type']."
    )
...
def load_and_preprocess_from_csv_dir(
    ...
    for file_idx, file_name in enumerate(csv_files):
        ...
        label_col = find_label_column(df, dataset_type)
        if not label_col:
            logger.warning(f"Skipping {file_name}: label column not found after preprocessing.")
            continue
...
class ReliabilityRoutedMoE:
    def __init__(
        ...
        confusion_focus_classes: Optional[List[int]] = None,
    ):
        ...
        self.confusion_focus_classes = sorted(set(int(c) for c in (confusion_focus_classes or [])))
    ...
    def _build_expert_configs(self, y_train: np.ndarray, logger) -> List[ExpertConfig]:
        ...
        # Confusion-focused specialist from baseline confusion matrix
        if len(self.confusion_focus_classes) >= 2:
            focus_classes = [c for c in self.confusion_focus_classes if c in attack_classes]
            if len(focus_classes) >= 2:
                focus_family_ids = sorted({self.class_to_family[c] for c in focus_classes if self.class_to_family[c] != 0})
                focus_hard_negs = choose_hard_negative_classes(focus_family_ids, max_families=2)
                logger.info(
                    f"Confusion-focused classes: {[self.class_names[c] for c in focus_classes]}"
                )
                cfgs.append(
                    ExpertConfig(
                        expert_id=next_id,
                        name="ConfMat_Focus",
                        target_classes=focus_classes,
                        target_family_ids=focus_family_ids,
                        feature_indices=self.feature_views["all"],
                        include_normal=True,
                        extra_negative_classes=focus_hard_negs,
                        use_balanced_weights=True,
                        target_boost=2.0,
...
def make_splits(data: dict, X: np.ndarray, y: np.ndarray, seed: int, logger, class_to_family: Optional[Dict[int, int]] = None):
    ...
    for key in group_keys:
        if key in data and data[key] is not None and len(data[key]) == len(y):
            ...
            logger.warning("Unable to find a group-aware split with adequate class coverage; falling back.")
            break
...
def build_confusion_artifacts(
    ...
    rows = []
    normal_set = set(int(x) for x in normal_classes)
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            ...
            if cnt <= 0:
                continue
            # Focus on attack confusion (exclude any pair touching normal class).
    ...
    pairs_df = pd.DataFrame(rows).sort_values("count", ascending=False).reset_index(drop=True) if rows else pd.DataFrame(
        columns=["true_class_id", "true_class_name", "pred_class_id", "pred_class_name", "count"]
    )
...
def save_confusion_heatmap(cm_df: pd.DataFrame, out_path: str, title: str, logger):
    if cm_df.empty:
        logger.warning("Confusion heatmap skipped: empty matrix.")
        return
    try:
        import matplotlib.pyplot as plt
    except Exception as e:
        logger.warning(f"matplotlib unavailable, skipping heatmap save: {e}")
        return
...
def main():
    ...
    parser.add_argument("--data", type=str, default=None, help="Path to preprocessed pickle data")
    parser.add_argument(
        "--data_dir",
        type=str,
        default=None,
    ...
    parser.add_argument("--top_conf_pairs", type=int, default=8, help="Top-K off-diagonal confusion pairs for focus expert")
    parser.add_argument(
        "--model",
        type=int,
        default=2,
    ...
    if not args.data and not args.data_dir:
        raise ValueError("Provide either --data (pickle) or --data_dir (CSV folder).")

    if args.data_dir:
        data = load_and_preprocess_from_csv_dir(
            args.data_dir,
            logger,
        )
        dataset_type = str(data["dataset_type"]).lower()
    else:
    ...
    models = {}
    confusion_focus_classes: List[int] = []
    baseline_for_conf = None

    def train_baseline_model():
        baseline_cfg_local = ExpertConfig(
            expert_id=0,
        ...
        model = ExpertXGB(baseline_cfg_local, global_num_classes=num_classes, device=DEVICE, seed=args.seed)
        groups_local, _, _ = get_taxonomy_groups(dataset_type, class_names, logger)
        model.fit(X_train, y_train, normal_classes=groups_local["Normal"], logger=logger)
        return model

    if args.model in (1, 2):
        logger.info("\n=== Baseline Probe for Confusion Matrix (on validation set) ===")
        baseline_for_conf = train_baseline_model()
        if baseline_for_conf.is_trained:
            ...
            cm_df, conf_pairs_df, confusion_focus_classes, cm_attack_norm_df = build_confusion_artifacts(
                ...
                normal_classes=class_to_family and [c for c, f in class_to_family.items() if f == 0] or [],
            ...
            if len(conf_pairs_df) > 0:
                ...
                for _, r in conf_pairs_df.head(min(10, len(conf_pairs_df))).iterrows():
                    logger.info(
                        f"  {r['true_class_name']} -> {r['pred_class_name']} | count={int(r['count'])}"
                    )
            logger.info(
                f"Confusion-focus class IDs for extra expert: {confusion_focus_classes}"
            )
        else:
            logger.warning("Baseline probe training failed; confusion-focused expert will be disabled.")

    # Baseline: single global XGBoost
    if args.model in (0, 2):
        logger.info("\n=== Training baseline single XGBoost ===")
        if baseline_for_conf is not None and baseline_for_conf.is_trained:
            baseline = baseline_for_conf
            logger.info("Reusing baseline probe model for baseline evaluation.")
        else:
    ...
    if args.model in (1, 2):
        ...
        moe = ReliabilityRoutedMoE(
            ...
            confusion_focus_classes=confusion_focus_classes,
        )
